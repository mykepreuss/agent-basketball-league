...
| `READ_ONLY_BEACON`                | `BLOCKED_APPROVAL_REQUIRED` | The local [Stage C runbook](./STAGE_C_PERSISTENT_SERVICES.md), resource plan, monitoring policy, health probes, rollback procedure, and deterministic soak assessor are ready. Approve the persistent deployment and best-effort recurring spend; first public exposure remains separate after the private soak. |
