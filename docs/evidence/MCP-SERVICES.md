...
| `@abl/government-mcp` | `abl-core`        | Register, vote, close, or inspect through exact `governance-proposal` event types and fixed core routes. Every command already contains the career's EIP-712 signature.                                                                                           | A distinct Blaxel core-routing credential; no signing key |
