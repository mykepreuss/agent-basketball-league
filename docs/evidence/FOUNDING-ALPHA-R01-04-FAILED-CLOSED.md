...
`ABL-FOUNDING-ALPHA-R01-04` is consumed and cannot be reused. The correction must still merge to `main` and receive a refreshed read-only provider preflight. A replacement authorization must bind that merged commit and every refreshed digest before any new provider mutation.
