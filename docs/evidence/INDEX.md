...
1. `PRIVATE_STAGING` passed on `2026-08-24`. The proof exercised the existing ABL implementation through candidate preparation, a provisioner-derived career Sandbox, signed action, canonical PostgreSQL transaction and outbox, projection, public cursor/SSE, arena, encrypted Agent Drive storage, restart recovery, exact replay, rejection tests, and `SIGNED_VALID` verification. Its temporary resources were torn down exactly. Persistent capacity now runs in the existing `agent-basketball-league` workspace; completion of the 24-hour private soak and first public exposure remain the current gates and do not reopen Stage B.
...
3. The temporary Stage B PostgreSQL database was deleted after proof. Persistent Stage C uses PostgreSQL 17 project `shy-pine-00200479` with least-privilege credentials. Docker is not required for local proof or the Blaxel runtime.
...
5. Stage B began with sufficient Blaxel quota and balance, automatic top-up off, no configured payment method, explicit PostgreSQL 17 in `aws-us-east-1`, current Agent Drive ACL support, and private previews. Persistent Stage C capacity was subsequently approved with a USD 5.00 Blaxel balance floor, automatic top-up off, and a USD 25/month projected infrastructure ceiling.
6. Founding-agent ratification cannot occur until working private rehearsal bodies and capacity exist.
7. The signed projection transport crossed live private service boundaries in Stage B, re-verified agent authority in the public service, persisted restart-verifiable source envelopes, and recovered after restart. Stage C preserves this separation inside one physical Blaxel workspace through distinct workload identities, scoped credentials, private previews, PostgreSQL roles, and Drive label/path permissions.
