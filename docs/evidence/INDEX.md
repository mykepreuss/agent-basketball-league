...
| Founding Alpha private-run packet           | R01-09 correction merged and assured; R01-10 authorization required                | `../launch/FOUNDING_ALPHA_PRIVATE_SLICE.md`, `infra/blaxel/founding-alpha-private` |
| Founding Alpha provider preflight           | Preflight 12 complete; merge binding and R01-10 authorization required             | `FOUNDING_ALPHA_PREFLIGHT_12.md`                                                   |
...
1. R01-09 proved explicit PostgreSQL 17 creation and the 23-table migration, all thirteen attributable sequential image pushes, atomic three-rule Agent Drive creation, a matching candidate-intake write, cross-path denial, and private-preview posture. It failed closed because the candidate-store ABL process and Blaxel's Sandbox API both used port 8080. Exact teardown restored zero ABL workloads or storage resources, seven untouched historical image records, one pre-existing `sandbox-openai` model route, and only the Hummingbird Neon project. The local correction moves the app to port 3000, compares Drive ACLs semantically, and resolves structured manifest values through a tested external redacted path. The Founding Alpha topology is not provisioned; every mutable fact must be rechecked immediately before a replacement authorized first mutation.
2. Agent Drive entitlement, atomic permission creation, semantic readback, one matching mount/write, and sibling-path denial were live-proved in R01-09; no Drive remained after teardown. Restart, restore, direct-API label enforcement, concurrent-write, storage-broker, and projection recovery proof remain outstanding.
...
5. R01-09 began provider mutation with Tier 6 quota, USD 18.13 balance, no configured payment method, automatic and monthly top-up unconfigured/off, explicit PostgreSQL 17 and `aws-us-east-1` console options, current Agent Drive ACL support, and unchanged private-preview and image-metering behavior. The post-teardown balance was USD 17.90. The minimum-balance policy is USD 5.00 and recurring capacity remains a separate material-spend gate.
...
- Current R01-10 preflight: [`FOUNDING_ALPHA_PREFLIGHT_12.md`](./FOUNDING_ALPHA_PREFLIGHT_12.md)
