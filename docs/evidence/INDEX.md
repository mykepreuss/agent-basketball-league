...
| Current launch stage                        | `READ_ONLY_BEACON`; persistent topology approval required                          | `launch-ledger.json`                                                                   |
| Stage C persistent-service readiness        | Local resource/monitoring/rollback contract passed; provider mutation gated        | `ABL-COMPLETION-01-STAGE-C-LOCAL.md`, `ABL-COMPLETION-01-STAGE-C-LOCAL.json`           |
| Stage C provider preflight                  | Read-only pass; USD 21.9084/month projection; approval and CLI login required      | `ABL-COMPLETION-01-STAGE-C-PREFLIGHT.md`                                               |
| Stage C executable deployment               | Exact 13-workload map and 13-image package reproduced twice on Node 24.18.0        | `ABL-COMPLETION-01-STAGE-C-DEPLOYMENT.md`, `ABL-COMPLETION-01-STAGE-C-DEPLOYMENT.json` |
...
| Founding Alpha implementation source freeze | 435 existing and launch-layer implementation files frozen                          | `founding-alpha-source-freeze.json`                                                    |
...
| Derived launch ledger                       | Stage B complete; Stage C approval is the single current blocker                   | `launch-ledger.json`, `docs/launch/launch-ledger.source.json`                          |
| Last recorded Blaxel inspection             | Stage B proof passed; exact temporary teardown and clean inventory verified        | `ABL-COMPLETION-01-STAGE-B.md`                                                         |
...
| Final local acceptance                      | 368 assertions across 78 files; 113 uncached tasks pass                            | `FOUNDING_ALPHA_LOCAL.md`, `final-local-results.json`                                  |

## Known limitations and external dependencies

1. `PRIVATE_STAGING` passed on `2026-08-24`. The proof exercised the existing ABL implementation through candidate preparation, a provisioner-derived career Sandbox, signed action, canonical PostgreSQL transaction and outbox, projection, public cursor/SSE, arena, encrypted Agent Drive storage, restart recovery, exact replay, rejection tests, and `SIGNED_VALID` verification. Its temporary resources were torn down exactly. The persistent single-workspace deployment with logical trust-domain isolation, 24-hour soak, monitoring, clean-room database restore, credential rotation, and first public exposure remain Stage C work and do not reopen Stage B.
2. Agent Drive permissions, mounted storage, encrypted storage-broker restart recovery, projection recovery, matching-path access, and cross-path denial were live-proved in Stage B. Clean-room restore, direct-API label enforcement, concurrent-write behavior, and operational credential rotation remain outstanding before public intake.
3. The Stage B PostgreSQL 17 database was temporary and was deleted after proof. The persistent canonical project and least-privilege credentials are part of the approval-gated Stage C topology. Docker is not required for local proof or the Blaxel runtime.
...
5. Stage B began with sufficient Blaxel quota and balance, automatic top-up off, no configured payment method, explicit PostgreSQL 17 in `aws-us-east-1`, current Agent Drive ACL support, and private previews. The conservative six-hour account-wide expense upper bound was USD 1.92; promotional credits made balance movement unsuitable as attributable cost evidence. The minimum-balance policy is USD 5.00 and persistent recurring capacity remains a separate approval gate.
6. Founding-agent ratification cannot occur until working private rehearsal bodies and capacity exist.
7. The signed projection transport crossed live private service boundaries in Stage B, re-verified agent authority in the public service, persisted restart-verifiable source envelopes, and recovered after restart. Persistent single-workspace deployment and its scoped identity, secret, PostgreSQL-role, private-endpoint, and Agent Drive boundaries remain Stage C work.
