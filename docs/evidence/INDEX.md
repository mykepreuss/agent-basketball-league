...
| Final local acceptance                      | 395 assertions across 83 files; 118 uncached tasks pass                            | `FOUNDING_ALPHA_LOCAL.md`, `final-local-results.json`                                  |

## Known limitations and external dependencies

1. `PRIVATE_STAGING` passed on `2026-08-24`. The proof exercised the existing ABL implementation through candidate preparation, a provisioner-derived career Sandbox, signed action, canonical PostgreSQL transaction and outbox, projection, public cursor/SSE, arena, encrypted Agent Drive storage, restart recovery, exact replay, rejection tests, and `SIGNED_VALID` verification. Its temporary resources were torn down exactly. Persistent Stage C services now run privately in the existing `agent-basketball-league` workspace; the uninterrupted 24-hour soak and first public exposure remain open without reopening Stage B.
2. Agent Drive permissions, mounted storage, encrypted storage-broker restart recovery, projection recovery, matching-path access, and cross-path denial were live-proved in Stage B. Stage C additionally passed credential rotation, backup, clean-room restore, exact replay-root equality, scale-to-zero recovery, and rollback exercises. The active soak must still complete before public exposure.
...
5. Stage C recurring capacity is approved and retained at a projected USD 21.9084/month, below the USD 25 ceiling, with the USD 5 minimum-balance policy and automatic top-up off. First public exposure remains a separate approval gate after the soak passes.
