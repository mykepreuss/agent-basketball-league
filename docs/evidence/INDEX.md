...
| Founding Alpha provider preflight           | Read-only Preflight 10 complete; R01-08 authorization required                     | `FOUNDING_ALPHA_PREFLIGHT_10.md`                                                   |
...
1. R01-07 pushed and individually verified the thirteen exact images, then failed closed when the temporary Neon creation returned PostgreSQL 18 instead of the authorized PostgreSQL 17. Exact teardown restored zero ABL workloads or storage resources, seven untouched historical image records, one pre-existing `sandbox-openai` model route, and only the Hummingbird Neon project. The merged correction makes explicit PostgreSQL 17 console creation and exact readback the first provider mutation, before any image push. The Founding Alpha topology is not provisioned; every mutable fact must still be rechecked immediately before a replacement authorized first mutation.
