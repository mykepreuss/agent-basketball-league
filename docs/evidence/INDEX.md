...
| Final local acceptance                      | 366 assertions across 78 files; 113 uncached tasks pass                            | `FOUNDING_ALPHA_LOCAL.md`, `final-local-results.json`                              |

## Known limitations and external dependencies

1. R01-08 proved explicit PostgreSQL 17 console creation, exact project readback, and the 23-table migration as the first provider gate. It then failed closed because `bl push -d <context>` resolved the repository-root body image under the core and public image names. Exact teardown restored zero ABL workloads or storage resources, seven untouched historical image records, one pre-existing `sandbox-openai` model route, and only the Hummingbird Neon project. The local correction runs each push from its exact verified working directory without `-d`, requires a passing prior receipt, and rejects unattributable builds. The Founding Alpha topology is not provisioned; every mutable fact must be rechecked immediately before a replacement authorized first mutation.
...
5. Preflight 10 on `2026-08-22` local time showed Tier 6 quota, USD 18.26 balance, no configured payment method, automatic and monthly top-up unconfigured/off, low-balance alert at USD 5.00, explicit PostgreSQL 17 and `aws-us-east-1` console options, current Agent Drive ACL documentation, and unchanged private-preview and image-metering behavior. The minimum-balance policy is USD 5.00 and recurring capacity remains a separate material-spend gate.
...
- Current replacement preflight: [`FOUNDING_ALPHA_PREFLIGHT_10.md`](./FOUNDING_ALPHA_PREFLIGHT_10.md)
