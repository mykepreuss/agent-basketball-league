...
| Founding Alpha provider preflight           | Consumed by R01-07; replacement preflight required                                 | `FOUNDING_ALPHA_PREFLIGHT_09.md`                                                   |
...
5. R01-07's final teardown readback on `2026-08-22` local time showed Tier 6 quota, USD 18.33 balance, no configured payment method, automatic and monthly top-up unconfigured/off, and low-balance alert at USD 5.00. A replacement must prove a PostgreSQL 17 creation path or separately review and authorize PostgreSQL 18, then reverify region availability, Agent Drive ACL behavior, image metering, private-preview behavior, balance, and projected four-hour cost before mutation. The minimum-balance policy is USD 5.00 and recurring capacity remains a separate material-spend gate.
