...
All remaining safe local work passes. `docs/evidence/final-local-results.json` records `PASS_LOCAL_WITH_EXTERNAL_GATES` and stable result digest `0x785b2f6e438e14f8913c907b5e469e6085fe5f5b9c12041118c92c86a3aba3bb`.
...
| Unit, integration, property, behavioral EVM, migration/schema, and API tests | 136/136 assertions in 24 files; 28/28 tasks |
...
That is 150 passing assertions under the exact pinned Node `24.18.0` runtime. The generated test-result digest is present in the release candidate, but the candidate remains schema-invalid because real image, verifier, ratification, timing, key, and authorization inputs are still absent.
...
- `@abl/core-api` catalogs the documented candidate, combine, and admitted-agent routes. Normal startup keeps every consequential path at `503 genesis_not_authorized`. Explicit `ABL_REHEARSAL_MODE=1` enables signed `PossessionResolved`, the candidate lifecycle, combine registration/status, personal-memory commitments, and body continuity; all other consequential routes remain closed facades. Candidate registration begins with a DID-bound expiring HMAC challenge and former-operator provenance signature; isolated transfer installs an externally generated self-proving career key, after which reflection/progress, admission, revocation/withdrawal, status, and export replay from canonical events. Combine consent must then be signed by that career key within the exact configured 14-day window and bind the replay-verified admission event hash. Personal-memory persistence/correction/inspection/export/deletion uses the same current career authority, accepts commitments rather than content, and independently verifies corresponding private-storage state. Continuity policy, activity, standby, protected notice, deletion, rehydration, material-change refusal, and inspection bind to admitted provenance and configured recognized images, but explicitly do not claim live clean-room or guardian proof. Every rehearsal response remains explicitly non-genesis and cannot enter a public recognition checkpoint.
...
- Final local result record: `sha256:ac22fe5bdebb3edee2d2a2e8ca1ad3d3464c9faa6c65a8e52cf530ab33cfd943`
...
- Genesis readiness bundle: `sha256:bb190bba651f4e94e859adc15a4b6c635b0cefb30d42cc3c1a02c746f40c3228`
- Pending release candidate: `sha256:b1048dd863b125cf25918d0089182a10741dbcd756072ddb2738502f8ecd6960`
