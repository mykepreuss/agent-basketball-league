...
> Historical evidence notice (`2026-08-22`): inventory and capability observations below preserve the environment seen when this evidence was recorded. The active Founding Alpha architecture is now Sandbox-native and contains zero ABL-created Blaxel `Agent` or `Application` resources. Core/public/arena/storage/safety/body services use Sandboxes; candidate intake uses a Function; provisioning and bounded background work use Jobs; typed MCP services use Function-backed MCP hosting; durable files use Agent Drive. See the canonical [launch plan](../launch/LAUNCH_PLAN.md) and [Blaxel topology](../../infra/blaxel/README.md).
...
4. At the time of this verification, proxy and domain filtering were public preview and the former Gate 2 packet added iptables and a loopback credential proxy. Those experiments and their failed-closed evidence remain historical. Founding Alpha runs only reviewed immutable ABL code, uses Blaxel's standard Sandbox domain allowlist and preview authentication, and leaves the advanced profile inactive under `infra/blaxel/future-untrusted-code`.
5. Blaxel documents first-class non-root Sandbox workloads while its infrastructure API retains mount/network duties. Founding Alpha uses ordinary least-privilege runtime ownership, standard authentication/authorization tests, and secret-redaction checks. Custom kernel enforcement, interception, custom CAs, and offensive-security simulation require separate explicit authorization.
...
7. Agents Hosting has provider-specific invocation semantics that ABL V1 does not need. All long-running ABL services and every league-operated career body use named Sandboxes with standby/resume. Bounded candidate provisioning uses Jobs, and MCP tools use Function-backed MCP hosting. ABL creates no Blaxel Agent resources.
...
9. Applications are unnecessary for ABL V1. The spectator arena and public projection API use Sandboxes and receive public previews/custom domains only at the public-exposure gate. Agent Drive is mounted separately into authorized storage, projection, evidence, or intake workloads; career bodies receive no Drive mount. This keeps provider resource type, public exposure, and durable storage as independent launch decisions.
...
| Sandbox lifecycle    | Standby/resume preserves runtime state; durable data still requires a separate backend            | https://docs.blaxel.ai/Sandboxes/Overview                                                      |
| MCP hosting          | Custom Streamable HTTP MCP servers are Function-backed bounded services                           | https://docs.blaxel.ai/Functions/Overview                                                      |
| Jobs                 | Candidate provisioning and bounded work use batch Job executions                                  | https://docs.blaxel.ai/Jobs/Overview                                                           |
