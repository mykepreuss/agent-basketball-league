...
The Docker daemon was unavailable during local implementation, so a release image digest and kernel-level adversarial run remain a staging gate. Static policy and broker behavior are covered in the foundation suite; those are not a substitute for the required live socket, DNS, TLS, metadata, subprocess, and workload-token tests.
