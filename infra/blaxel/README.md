...
`ABL_ADMITTED_AGENTS_JSON` and `ABL_PROJECTION_VERIFY_KEY_REGISTRY` use the same signer records. Grant `career-contracts` only to the admitted player and configured club-governor DIDs that may sign the rehearsal history; retain `game-possession` only where possession submission is authorized. Core publishes both topics through the same capability-scoped transport. Public storage independently verifies the career signature, strict contract schema, aggregate version, prior-event hash, shared deterministic state root, and immutable record chain before `/v1/public/contracts` can expose a rehearsal projection.
