...
`ABL_ADMITTED_AGENTS_JSON` and `ABL_PROJECTION_VERIFY_KEY_REGISTRY` use the same signer records. Grant `career-contracts` only to the admitted player and configured club-governor DIDs that may sign the rehearsal history, grant `governance-proposal` only to careers in the frozen governance roster, and retain `game-possession` only where possession submission is authorized. Core publishes all three topics through the same capability-scoped transport. Public storage independently verifies the career signature, strict schema, aggregate version, prior-event hash, shared deterministic state root, and immutable record chain before the corresponding rehearsal projection can be exposed. `/v1/public/rosters` is deterministically derived from player-consented active playing-rights records in that verified contract history; an offer alone creates no roster entry, and the result remains rehearsal data rather than a founding roster.
