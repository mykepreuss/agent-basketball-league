...
Run `pnpm contract:prepare -- <config.json> <output.json>` to compile with pinned Solidity `0.8.36` and prepare a Base Sepolia contract-creation transaction. The command is deliberately prepare-only: it rejects any chain other than `84532`, never reads a private key or RPC URL, and never broadcasts. The example values are non-secret placeholders and are not genesis authorization.
